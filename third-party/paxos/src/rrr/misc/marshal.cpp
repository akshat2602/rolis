...
namespace rrr
{
    ...
    {
        // Log_debug("bypassing copying %d", sz);
        ...
        {
            // Log_debug("resizing current chunk %d", tail_->write_idx);
        ...
        {
            Log_debug("alarm alarm alarm 3");
        }
        write_cnt_ += sz;
        content_size_ += sz;
        assert(content_size_ == content_size_slow());
        // Log_debug("final size is %d", write_cnt_);
        return sz;
    }
    ...
    size_t Marshal::peek(void *p, size_t n) const
    {
        assert(tail_ == nullptr || tail_->next == nullptr);
        assert(empty() || (head_ != nullptr && !head_->fully_read()));
        // Log_debug("is peeking empty %d %d", empty(), n);
        char *pc = (char *)p;
        size_t n_peek = 0;
        chunk *chnk = head_;
    ...
    size_t Marshal::chnk_read_from_fd(int fd, size_t bytes)
    {
        ...
        //         Log_debug("alarm alarm alarm 4");
        //         Log_debug("********* write_cnt_: %d", write_cnt_);
        //         std::cout << "******* value of bytes" << bytes << std::endl;
        //         std::cout << "******* read bytes" << read_bytes << std::endl;
        //         Log_debug("********* read_bytes: %ld", read_bytes);
        //     }
        if (read_bytes == -1)
        {
            Log_debug("alarm alarm alarm 4");
            Log_debug("********* write_cnt_ before: %d with fd: %d", write_cnt_, fd);
        }
        ...
        {
            Log_debug("alarm alarm alarm 4");
            Log_debug("********* write_cnt_ after: %d with fd: %d", write_cnt_, fd);
        }
        // Log_debug("********* write_cnt_: %d", write_cnt_); // kshivam marker, uncomment
        // if(read_bytes <= 0){
    ...
    size_t Marshal::write_to_fd(int fd)
    {
        ...
        while (!empty())
        {
            ...
            // Log_debug("written %d bytes of %d", head_->read_idx, head_->write_idx);
            if (head_->fully_read())
            {
                if (head_ == tail_)
                {
                ...
                // Log_debug("fully read a chunk of size %d %d", head_->data->size, head_->write_idx);
                chunk *chnk = head_;
                head_ = head_->next;
                delete chnk;
                ok = true;
            ...
            if (cnt <= 0)
            {
                // Log_debug("written less than 0 bytes, breaking... %d %d %d", head_->data->size, head_->write_idx, head_->read_idx);
                break;
            }
            else
            {
                // Log_debug("written %lld bytes of %lld", head_->read_idx, head_->write_idx);
            }
