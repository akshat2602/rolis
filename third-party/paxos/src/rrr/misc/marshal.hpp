...
namespace rrr
{
  ...
  public:
    ...
    virtual size_t WriteToFd(int fd, int written_to_socket)
    {
      ...
      if (written_to_socket < sizeof(kind_))
      {
        ...
        // Log_debug("Writing the kind of MarshallDeputy %d %d", sz, written_to_socket);
        written_to_socket += sz;
        if (written_to_socket < sizeof(kind_))
          return sz;
      }
      // Log_debug("Written bytes of ghost chunk 1 %d %d %d", sz, kind_, written_to_socket);
      //  sp_data_.get()->reset_write_offset();
      sz = sp_data_.get()->WriteToFd(fd, written_to_socket - sizeof(kind_));
      // std::cout << sz << std::endl;
      // Log_debug("Written bytes of ghost chunk 2 %d %d", sz, kind_);
      written_to_socket += sz;
      // Log_debug("Written bytes of ghost chunk 3 %d %d %d", written_to_socket, kind_, EntitySize());
      // Log_debug("Written bytes of ghost chunk 2 %d %d", written_to_socket, kind_);
  ...
  class Marshal : public NoCopy
  {
    struct raw_bytes : public RefCounted
    {
      ...
      raw_bytes(MarshallDeputy md, size_t sz)
      {
        ...
        size = sz;
        shared_data = true;
        // Log_debug("Creating a ghost chunk here of size %d of kind %d", sz, md.kind_);
      }
    ...
    public:
      ...
      int write_to_fd(int fd)
      {
        ...
        if (data->shared_data)
        {
          ...
          // Log_debug("wrote %d bytes of ghost %d", cnt, fd);
        }
        else
        {
          cnt = ::write(fd, data->ptr + read_idx, write_idx - read_idx);
          // Log_debug("wrote %d bytes of normal %d", cnt, fd);
        }
#ifdef RPC_STATISTICS
        if (!data->shared_data)
      ...
      int read_from_fd(int fd, size_t bytes = -1)
      {
        ...
        {
          Log_debug("@@@@alarm - should this occur?");
        }
        ...
        if (cnt == -1)
        {
          Log_debug("there seems to be some error while reading %s", strerror(errno));
          Log_debug("printing the stack: bytes: %ld, write_idx: %ld, data->size: %ld, data->ptr=null?: %d, read_idx: %ld", bytes, write_idx, data->size, data->ptr == nullptr, read_idx);
          return 0;
        }

        // Log_debug("####value being returned is %d", cnt);
        return cnt;
      }
      ...
      bool fully_read() const
      {
        assert(write_idx <= data->size);
        assert(read_idx <= write_idx);
        // Log_debug("fully read %d %d", read_idx, data->size);
  ...
  {
    char byte0;
    // Log_debug("peeking data of %d", m.peek(&byte0, 1));
    verify(m.peek(&byte0, 1) == 1);
    size_t bsize = rrr::SparseInt::buf_size(byte0);
    char buf[9];
    verify(m.read(buf, bsize) == bsize);
  ...
  inline rrr::Marshal &operator<<(rrr::Marshal &m, const rrr::MarshallDeputy &rhs)
  {
    ...
    {
      // Log_debug("size is %d", rhs.EntitySize());
      m << rhs.kind_;
